...
async def run_agent_loop(user_input: str, context: AgentContext) -> str:
    ...
    for round_num in range(context.max_rounds):  # 默认 10 轮
        # 1.5 上下文压缩检查（借鉴 Claude Code 的 AutoCompact 模式）
        #     当 token 消耗接近上限时，自动调用 default 模型生成摘要压缩历史
        if context.short_term_memory.needs_compression():
            summary = await llm.chat([...], role="default")
            context.short_term_memory.compress(summary)

        # 2. 选择本轮使用的模型角色
        role = model_router.select_role(context)

        # 3. 组装 prompt + 动态工具列表
        #    系统提示词 = 基础指令 + 环境信息 + 完整工具描述 + 活跃 Skill 上下文
...
### 4.8 上下文压缩策略（借鉴 Claude Code）

**决策**：单层自动压缩（ShortTermMemory 集成 AgentContext）

Claude Code 使用三层压缩（MicroCompact → AutoCompact → FullCompact），但对 MiniClaw 来说过度复杂。
我们采用简化版：
...
if context.short_term_memory.needs_compression():
    # token 超过上限 80% 时触发
    # 调用 default 模型（省钱）生成历史摘要
    summary = await llm.chat(compress_prompt, role="default")
    # 保留 system prompt + 摘要 + 最近 4 条消息
    context.short_term_memory.compress(summary)
```

**不做的**：
- MicroCompact（本地裁剪旧工具输出）— 用工具输出截断替代
- FullCompact（全对话摘要 + 重新注入文件）— 复杂度太高

### 4.9 系统提示词工程（借鉴 Claude Code）

**决策**：模板化系统提示词，动态拼装

Claude Code 的系统提示词采用静态/动态分离策略以最大化 Prompt Cache 命中率。
MiniClaw 不需要 Cache 优化，但借鉴其"完整的工具描述 + 环境上下文 + 行为约束"理念：

```python
SYSTEM_PROMPT = f"""你是 MiniClaw 🦞，运行在用户本地电脑上的 AI 助手。
...
### 网络
- web_search / http_request: 搜索和请求
### 浏览器（Playwright）
- browser_open / browser_action: 打开网页、点击输入
### 桌面操控（macOS）
- screen_capture / screen_analyze / mouse_click / keyboard_type
